...
class ClipMatcher(SetCriterion):
    def __init__(self, num_classes,
        ...
        self.base_weight_dict = dict(weight_dict)
    ...
    @staticmethod
    def _is_tracking_loss_key(loss_name: str) -> bool:
        name = loss_name.lower()
        if 'reid' in name:
            return False
        return ('loss_ce' in name) or ('loss_bbox' in name) or ('loss_giou' in name)

    @staticmethod
    def _is_reid_total_key(loss_name: str) -> bool:
        name = loss_name.lower()
        if 'reid' not in name:
            return False
        return ('reid_total' in name) or ('reid_loss' in name)

    @staticmethod
    def _is_reid_component_key(loss_name: str) -> bool:
        name = loss_name.lower()
        if 'reid' not in name:
            return False
        return ('reid_ce' in name) or ('reid_triplet' in name)

    def _append_uncertainty_loss(self, losses: dict) -> dict:
        if len(losses) == 0:
            return losses

        ref_loss = next(iter(losses.values()))
        tracking_loss = ref_loss.new_zeros(())
        reid_total_loss = ref_loss.new_zeros(())
        reid_component_loss = ref_loss.new_zeros(())
        num_tracking_terms = 0
        num_reid_total_terms = 0
        num_reid_component_terms = 0

        for key, value in losses.items():
            coef = float(self.base_weight_dict.get(key, 1.0))
            if self._is_tracking_loss_key(key):
                tracking_loss = tracking_loss + coef * value
                num_tracking_terms += 1
            elif self._is_reid_total_key(key):
                reid_total_loss = reid_total_loss + coef * value
                num_reid_total_terms += 1
            elif self._is_reid_component_key(key):
                reid_component_loss = reid_component_loss + coef * value
                num_reid_component_terms += 1

        if num_reid_total_terms > 0:
            reid_loss = reid_total_loss
        else:
            reid_loss = reid_component_loss

        num_reid_terms = num_reid_total_terms + num_reid_component_terms
        if num_tracking_terms == 0 and num_reid_terms == 0:
            return losses

        # Fallbacks keep training stable when one branch is temporarily missing.
        if num_tracking_terms == 0:
            uncertainty_total = reid_loss
        elif num_reid_terms == 0:
            uncertainty_total = tracking_loss
        else:
            weighted_tracking = 0.5 * torch.exp(-self.log_var_tracking) * tracking_loss
            weighted_reid = 0.5 * torch.exp(-self.log_var_reid) * reid_loss
            regularizer = 0.5 * (self.log_var_tracking + self.log_var_reid)
            uncertainty_total = weighted_tracking + weighted_reid + regularizer

            losses['loss_uncertainty_tracking_weighted'] = weighted_tracking.detach()
            losses['loss_uncertainty_reid_weighted'] = weighted_reid.detach()
            losses['loss_uncertainty_regularizer'] = regularizer.detach()

        losses['loss_tracking_for_uncertainty'] = tracking_loss.detach()
        losses['loss_reid_for_uncertainty'] = reid_loss.detach()
        losses['loss_uncertainty_total'] = uncertainty_total

        if self.use_uncertainty_loss and hasattr(self, 'log_var_tracking'):
            losses['uncertainty_log_var_tracking'] = self.log_var_tracking.detach()
            losses['uncertainty_log_var_reid'] = self.log_var_reid.detach()
    ...
    def forward(self, outputs, input_data: dict):
        ...
        if self.use_uncertainty_loss:
            losses = self._append_uncertainty_loss(losses)
        return losses
...
class MOTR(nn.Module):
    def __init__(self, backbone, transformer, num_classes, num_queries, num_feature_levels, criterion, track_embed,
                 ...
                 track_query_history_len=0, track_query_queue_score_thresh=0.0,
                 use_reid_query: bool = False, reid_num_ids: int = 2048,
        ...
        self.track_query_queue = None
        self._queue_frame_idx = 0
        ...
        self._reid_obj_to_cls = {}
        self.reid_queue_bank = None
    ...
    def _reset_track_query_queue(self):
        self._queue_frame_idx = 0
    ...
    def _build_reid_target_ids(self, track_instances: Instances) -> Optional[Tensor]:
        ...
        for obj_id in alive_obj_ids.tolist():
            oid = int(obj_id)
            if oid not in self._reid_obj_to_cls:
                self._reid_obj_to_cls[oid] = len(self._reid_obj_to_cls) % self.reid_num_ids
            targets.append(self._reid_obj_to_cls[oid])
        return torch.as_tensor(targets, dtype=torch.long, device=track_instances.obj_idxes.device)

    def _update_track_query_queue(self, track_instances: Instances):
        if self.track_query_queue is None:
            return
        self.track_query_queue.push(
            cam_idx=0,
            track_instances=track_instances,
            frame_id=self._queue_frame_idx,
        )
        self._queue_frame_idx += 1

    def get_track_query_queues(self, cam_idx=0):
        if self.track_query_queue is None:
            return {}
    ...
    def _post_process_single_image(self, frame_res, track_instances, is_last):
        ...
        if self.reid_module is not None and self.reid_queue_bank is not None:
            target_ids = self._build_reid_target_ids(track_instances) if self.training else None
            track_instances, reid_loss = self.reid_module(
                track_instances=track_instances,
                queue_bank=self.reid_queue_bank,
                cam_idx=0,
                global_frame_idx=self._queue_frame_idx,
                target_ids=target_ids,
            )
            if self.training and reid_loss is not None:
                frame_idx = max(0, self.criterion._current_frame_idx - 1)
                if isinstance(reid_loss, dict):
                    if 'total' in reid_loss:
                        self.criterion.losses_dict[f'frame_{frame_idx}_reid_total'] = reid_loss['total']
                    if 'ce' in reid_loss:
                        self.criterion.losses_dict[f'frame_{frame_idx}_reid_ce'] = reid_loss['ce']
                    if 'triplet' in reid_loss:
                        self.criterion.losses_dict[f'frame_{frame_idx}_reid_triplet'] = reid_loss['triplet']
                else:
                    self.criterion.losses_dict[f'frame_{frame_idx}_reid_total'] = reid_loss

        self._update_track_query_queue(track_instances)
...
def build(args):
    ...
    if getattr(args, 'use_reid_query', False):
        for i in range(num_frames_per_batch):
            weight_dict.update({"frame_{}_reid_total".format(i): args.reid_loss_coef})
    ...
    criterion.to(device)
    postprocessors = {}
    track_query_queue_len = getattr(args, 'track_query_queue_len', getattr(args, 'track_query_history_len', 0))

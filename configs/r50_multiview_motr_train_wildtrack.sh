...
set -eu
# Usage:
#   sh configs/r50_multiview_motr_train_wildtrack.sh "0,1" /path/to/wildtrack_mvmot ./output/wildtrack_motr_5cam
# If GPU_IDS is omitted, this script will use all visible GPUs.
SCRIPT_DIR=$(cd "$(dirname "$0")" && pwd)
REPO_ROOT=$(cd "$SCRIPT_DIR/.." && pwd)
GPU_IDS=${1:-""}
MOT_PATH=${2:-"$REPO_ROOT/wildtrack_mvmot"}
OUTPUT_DIR=${3:-"$REPO_ROOT/output/wildtrack_motr_5cam"}
...
# Progressive temporal context keeps five-camera training within the memory
# budget: 1 frame (epochs 0-19), 2 frames (20-59), then 3 frames (60-99).
# The maximum sample contains 5 cameras x 3 frames = 15 images per DDP rank.
...
set -- \
    ...
    --wandb \
    --wandb_project fusiontrack_wildtrack_5cam \
    --num_cams 5 \
